import { Human, Point } from "@vladmandic/human";
...
const colorDepth = (z: number | undefined): string => {
...
const HumanDetectionDisplay: React.FC<Props> = ({
	...
	useEffect(() => {
		if (canvasRef.current && detectedHuman) {
			...
			const result = detectedHuman.next(detectedHuman.result);
			if (result.face && result.face![0]) {
				...
				ctx.lineWidth = 1;
				ctx.beginPath();
				const centerX = canvasDrawWidth / 2.0;
				const centerY = canvasDrawHeight / 2.0;
				const meanX = face.box[0] + face.box[2] / 2.0;
				const meanY = face.box[1] + face.box[3] / 2.0;
				const xShift = centerX - meanX;
				const yShift = centerY - meanY;

				const shiftedMesh = face.mesh.map((p) => {
					const shifted = [p[0] + xShift, p[1] + yShift, p[2]] as Point;
					return shifted;
				});
				if (shiftedMesh.length >= 468) {
					for (let i = 0; i < TRI468.length / 3; i++) {
						...
						for (let i = 1; i < points.length; i++) {
							ctx.strokeStyle = colorDepth(points[i][2] || 0);
